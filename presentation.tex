...
% === PAIK POPULATION TABLE (SLIDE 1) ===
\begin{frame}
\slidetitle{Paik Population Estimates (Standard Mobilization Scenario)}
...
\toprule
\textbf{Mobilized Paiks (millions)} & \textbf{Male Population (millions)} & \textbf{Total Population (millions)} \\
\midrule
0.26 & 1.04 & 4.16 \\
0.36 & 1.44 & 5.76 \\
0.40 & 1.60 & 6.40 \\
...
\infobox{The Militia State}{%
  King Rudra Singha's militia register revealed that 260,000 to 400,000 \textit{paiks} could be mobilized. By assuming a standard draft of one man per four-person \textit{got}, we see a massive demographic base subjected to corv\'{e}e labor.
}

\column{0.48\textwidth}
\critiquebox{Overwhelmingly Rural}{%
  The urban content of the population was extremely low, with less than an estimated 2\% living in towns. A developed money economy---including bankers, insurers, and brokers---was conspicuously absent even by 1750.
}
\end{columns}
\end{frame}

% === PAIK AUTHORITY & ELITE CLASS TABLE (SLIDE 2) ===
\begin{frame}
\slidetitle{The Ahom Administrative Classes \& Paik Authority}

\begin{columns}[T]
\column{0.5\textwidth}
\textbf{\color{rust}The Static Elite (Based on 260k \textit{Gots})}
...
\arrayrulecolor{warmgray}
\toprule
\textbf{Official} & \textbf{Command (\textit{Gots})} & \textbf{Est. Officers} \\
\midrule
\textbf{Phukan} & 6,000 & $\sim$43 \\
\textbf{Rajkhowa} & 3,000 & $\sim$87 \\
\textbf{Hazarika} & 1,000 & 260 \\
\textbf{Saikia} & 100 & 2,600 \\
\textbf{Bora} & 20 & 13,000 \\
\midrule
\textbf{Total} & & \textbf{$\sim$15,990} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{6pt}
{\small\color{slate}Because officers held territorial command over the entire 4-man \textit{got} rather than just the mobilized labor, the administrative class was capped at roughly 1.5\% of the adult male population.}

\column{0.45\textwidth}
\thesisbox{Surplus Extraction \& Status}{%
  Ministers and officers were not salaried. Instead, they extracted surplus directly through \textit{likchaus} (servitors) taken from the \textit{paik} units they commanded. Peasant \textit{paiks} promoted to petty offices (like Bora or Saikia) gained \textit{chamua} status, completely exempting them from manual labor.
}
