...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \fill[ink] ([yshift=-30pt]current page.north west) rectangle ([xshift=55pt, yshift=-200pt]current page.north west);
  % Thin gold line
  \draw[ochre, line width=1.5pt] ([xshift=65pt, yshift=-40pt]current page.north west) -- ([xshift=65pt, yshift=-190pt]current page.north west);
  % Bottom strip
  \fill[clay] (current page.south west) rectangle ([yshift=20pt]current page.south east);
\end{tikzpicture}
...
\begin{itemize}
  \item \textbf{Post-Independence Era:} Assam boasted a per capita income higher than the national average, driven by primary resources and a strong tea export industry.
  \item \textbf{The Decline:} By 1986, per capita income slipped below the national average.
  \item \textbf{Current State:} The deficit widened to 45\% by the 1990s.
\end{itemize}

\column{0.45\textwidth}
\critiquebox{Structural Stagnation}{%
  Despite recent GSDP growth, Assam suffers from persistently low capital formation, sluggish industrialization, and a hypertrophied service sector unable to generate broad-based employment.
}
\end{columns}

\vspace{10pt}
\thesisbox{The Political Economy Thesis}{%
  Economic underdevelopment here is rarely an accident of geography alone; it is frequently the endogenous result of institutional arrangements and the distribution of political power among competing socio-economic classes.
}
\end{frame}
...
\begin{center}
  \begin{tikzpicture}[node distance=2cm, every node/.style={font=\footnotesize, align=center}]
    ...
    \node[anchor=north] at (9.5, -0.2) {Fiscal Crisis: \\ High Committed \\ Expenditure};
...
\begin{itemize}
  ...
  \item \textbf{Absence of a Salariat:} A salaried, Weberian middle class was virtually non-existent.
\end{itemize}

\column{0.4\textwidth}
\infobox{Historical Baseline}{%
  The old regime left behind a society deeply accustomed to state patronage (via land), but entirely unaccustomed to capital accumulation, commercial trade, or monetized administration.
}
\end{columns}
\end{frame}
...
\vspace{8pt}
\thesisbox{The Trade Deficit Problem}{%
...
\begin{tikzpicture}
  \node[
    ...
    {\bfseries\color{burgundy}The Administrative Shock}\\[4pt]
    \begin{itemize}
      \item[\color{burgundy}\rule[0.4ex]{4pt}{0.8pt}] British imposed a monetized economy and rational-legal bureaucracy.
      \item[\color{burgundy}\rule[0.4ex]{4pt}{0.8pt}] Assamese population was entirely lacking in modern Western education.
      \item[\color{burgundy}\rule[0.4ex]{4pt}{0.8pt}] Colonial state imported English-educated Bengali professionals for efficiency.
    \end{itemize}
  };
...
\begin{tikzpicture}
  \node[
    ...
    {\bfseries\color{deepgreen}The Assamese Response}\\[4pt]
    \begin{itemize}
      \item[{\color{deepgreen}\rule[0.4ex]{4pt}{0.8pt}}] Indigenous elites were profoundly marginalized (Bengali imposed as court language in 1836).
      \item[{\color{deepgreen}\rule[0.4ex]{4pt}{0.8pt}}] Elites rapidly adapted by embracing Western education to secure salaried state positions.
    \end{itemize}
  };
...
\critiquebox{A State-Dependent Class}{%
  Unlike European bourgeoisies or mercantile capitalists in western India, the Assamese middle class did not rise through mercantile trade or industrial innovation. Their prestige and security were directly tied to colonial state proximity and government salaries.
}
\end{frame}
...
\begin{frame}
\slidetitle{The Outsourcing of Capital}

\begin{columns}[T]
\column{0.5\textwidth}
\begin{tikzpicture}
  \node[
    draw=clay, line width=1pt, rounded corners=2pt,
    fill=parchment, inner sep=10pt, text width=0.9\linewidth, font=\small
  ] {%
    \textbf{\color{rust}Cultural Aversion to Enterprise}\\[6pt]
    The periodical \textit{Awahan} recorded that educated Assamese youth viewed trade and petty commerce with deep disdain, considering entrepreneurial labor to be a \textbf{``matter of big shame''}.\\[6pt]
    Instead of building businesses, youths wasted creative energies writing petitions to British officers begging for government posts.
  };
\end{tikzpicture}

\column{0.5\textwidth}
\begin{itemize}
  \item \textbf{The Economic Vacuum:} This cultural stigmatization completely prevented the Assamese from evolving into an independent capitalist class.
  \item \textbf{Foreign \& Immigrant Capital:} Millions of outsiders migrated to Assam to capitalize on this commercial void. Immigrant communities, notably the Marwaris, dominated trade, credit, and finance.
  \item \textbf{Wealth Drain:} This led to the systematic draining of local wealth and thoroughly stunted the growth of an indigenous entrepreneurial ecosystem.
\end{itemize}
\end{columns}
\end{frame}
...
\bottomrule
\multicolumn{2}{l}{\tiny\textit{Note: Reference data illustrating comparative literacy outcomes.}} \\
\end{tabular}
\end{center}
\end{small}
...
\begin{tikzpicture}
  \node[
    ...
    fill=cream, inner sep=8pt, font=\small
  ] {%
...
\vspace{10pt}
\begin{itemize}
  \item[\textcolor{rust}{\textbf{1.}}] \textbf{Path Dependency:} Assam's pre-colonial Paik system created a cultural affinity for state patronage rather than entrepreneurship.
  ...
  \vspace{8pt}
  \item[\textcolor{rust}{\textbf{3.}}] \textbf{Fiscal Capture:} Nearly 60\% of state revenue is consumed by salaries and pensions, starving productive capital investment.
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Accent strip
  \fill[clay] ([yshift=-45pt]current page.north west) rectangle ([xshift=55pt, yshift=-195pt]current page.north west);
  % Gold line
  \draw[ochre, line width=1.5pt] ([xshift=65pt, yshift=-55pt]current page.north west) -- ([xshift=65pt, yshift=-185pt]current page.north west);
  % Bottom bar
  \fill[clay!80!ink] (current page.south west) rectangle ([yshift=16pt]current page.south east);
\end{tikzpicture}
...
\hspace{60pt}\begin{minipage}{0.85\textwidth}
  {\fontsize{32}{36}\selectfont\color{white}\textbf{Thank You}}
  
  \vspace{8pt}
  {\color{ochre}\rule{50pt}{3pt}}
  
  \vspace{16pt}
  {\fontsize{16}{20}\selectfont\color{white}\bfseries Questions \& Discussion}
  
  \vspace{20pt}
  {\Large\color{white}\textbf{Kabeer Bora}}\\[4pt]
  {\large\color{white}University of Missouri--Kansas City}\\[4pt]
  
  \vspace{16pt}
  {\normalsize\color{white}\textit{Comments and suggestions are welcome.}}
\end{minipage}
...
\vspace{4pt}
{\small\color{white}\textbf{Download Slides}}
\end{columns}
\end{frame}
